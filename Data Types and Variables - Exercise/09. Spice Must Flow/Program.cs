...
namespace _09._Spice_Must_Flow
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            const int MINIMUM_SPICES_TO_GATER_FROM_MINE = 100;
            ...
            while(countOfSpices >= MINIMUM_SPICES_TO_GATER_FROM_MINE)
            {
                totalConsumed += countOfSpices - CONSUMED_BY_WORKERS;
                countOfSpices -= DAILY_DECREES_OF_MINE_YIELD;
                daysCounter++;

                if(countOfSpices < MINIMUM_SPICES_TO_GATER_FROM_MINE)
                {
